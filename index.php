...
class Initiator
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_bc_uatc_activate_license', array(Activation::class, 'activation_callback'));
        ...
        $addQuantityBoxOnShop = Options::bc_atc_get_option($uatcOptions, 'addQuantityBoxOnShop') === 'true';
        if ($addQuantityBoxOnShop) {
            add_filter('woocommerce_loop_add_to_cart_link', array($this, 'bc_uatc_add_quantity_fields'), 10, 2);
        }
        add_action('woocommerce_after_shop_loop_item', array($this, 'bc_uatc_show_stock_shop'), 10, 2);
        ...
        add_action('wp_footer', array($this, 'print_current_page'));
    ...
    public function bc_uatc_show_stock_shop()
    {
        global $product;
        if ($product->managing_stock()) {
            echo "<span style='display: none;' class='btc-uatc-max-qty'>" . $product->get_stock_quantity() . "</span>";
        }
    }

    public function bc_uatc_add_quantity_fields($html, $product)
    {
    ...
    public function print_current_page()
    {
        ?>
        <script>
            var bc_uatc_current_page = "<?php echo $this->get_current_page(); ?>";
        </script>
    ...
    {
        error_log('ajax_add_product_to_cart');
    ...
    public function add_custom_body_class($classes, $class)
    {
        ...
        $classes[] = $style_class;
        $customButtonClass = 'bc-uatc-custom-button';

        //check if user want to exclude button styles on certan pages
        $excludeShop = Options::bc_atc_get_option($uatcOptions, 'excludeShop') === 'true';
        $excludeSingle = Options::bc_atc_get_option($uatcOptions, 'excludeSingle') === 'true';
        $excludeCategory = Options::bc_atc_get_option($uatcOptions, 'excludeCategory') === 'true';
        $excludeTag = Options::bc_atc_get_option($uatcOptions, 'excludeTag') === 'true';
        $excludeOnOtherPages = Options::bc_atc_get_option($uatcOptions, 'excludeOtherPages') === 'true';


        error_log('excludeShop: ' . $excludeShop . ' excludeSingle: ' . $excludeSingle . ' excludeCategory: ' . $excludeCategory . ' excludeTag: ' . $excludeTag);
        if (is_shop() && !$excludeShop)
            $classes[] = $customButtonClass;

        if (is_product() && !$excludeSingle)
            $classes[] = $customButtonClass;

        if (is_product_category() && !$excludeCategory)
            $classes[] = $customButtonClass;

        if (is_product_tag() && !$excludeTag)
            $classes[] = $customButtonClass;
        if (!is_shop() && !is_product() && !is_product_category() && !is_product_tag() && !$excludeOnOtherPages)
            $classes[] = $customButtonClass;
        ...
        if (is_shop()) {
            $classes[] = 'bc-uatc-shop-page';
        } else if (is_product_category()) {
            $classes[] = 'bc-uatc-category-page';
        } else if (is_product_tag()) {
            $classes[] = 'bc-uatc-tag-page';
        } else if (is_product()) {
            $classes[] = 'bc-uatc-single-product-page';
        }
    ...
    public function save_options_cb()
    {
        ...
        $data = $data['options'];
        error_log("request: " . print_r($data, true));

        $settings_string = [];
        foreach ($data as $key => $value) {
            if (gettype($value) == 'string') {
                $settings_string[] = $key . "=" . sanitize_text_field($value);
            } else if (in_array(gettype($value), ['boolean', 'integer', 'float', 'double'])) {
                $settings_string[] = $key . "=" . $value;
            } else
                error_log('value is not supported' . gettype($value));
        }
        //check if the plugin is activated. If not, send a notice to the user
        if (!wp_verify_nonce($_POST["nonce"], "bc-uatc-save-settings-nonce")) {
            wp_die(__('Invalid nonce', ''));
        }
        if (!current_user_can('edit_posts')) {
            wp_die(__('You don not have the rights to continue', ''));
        }
        ...
        error_log("options are " . implode("&", $settings_string));
        //save to an option
        update_option(Config::BC_ULTIMATE_ATC_BUTTON_OPTIONS, implode("&", $settings_string));
    ...
    private function process_css()
    {
        ...
        $border_width = intval(Options::bc_atc_get_option($uatcOptions, 'buttonBorderWidth'));
        ...
        $textSize = intval(Options::bc_atc_get_option($uatcOptions, 'textSize'));
        ...
        */
        ...
        $useATCQuantityBox = Options::bc_atc_get_option($uatcOptions, 'useATCQuantityStyle') === 'true';
        if ($useATCQuantityBox && Config::IS_PRO) {
            ...
            $increaseDecreaseWidth = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddWidth'));
            $increaseDecreaseHeight = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddHeight'));

            $increaseDecreaseBorderColor = Options::bc_atc_get_option($uatcOptions, 'minusAddBorderColor');

            $increaseDecreaseBorderTop = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddBorderTop'));
            $increaseDecreaseBorderRight = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddBorderRight'));
            $increaseDecreaseBorderBottom = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddBorderBottom'));
            $increaseDecreaseBorderLeft = intval(Options::bc_atc_get_option($uatcOptions, 'minusAddBorderLeft'));

            $qtyBoxBorderColor = Options::bc_atc_get_option($uatcOptions, 'qtyBoxBorderColor');

            $qtyBoxBorderTop = intval(Options::bc_atc_get_option($uatcOptions, 'qtyBoxBorderTop'));
            $qtyBoxBorderRight = intval(Options::bc_atc_get_option($uatcOptions, 'qtyBoxBorderRight'));
            $qtyBoxBorderBottom = intval(Options::bc_atc_get_option($uatcOptions, 'qtyBoxBorderBottom'));
            $qtyBoxBorderLeft = intval(Options::bc_atc_get_option($uatcOptions, 'qtyBoxBorderLeft'));
            ...
            if ($increaseDecreaseWidth > 0) {
                $increase_decrease_input_rules .= sprintf('width: %1$s !important;', $increaseDecreaseWidth . 'px');
            }


            if (strlen($increaseDecreaseBorderColor) == 7) //rgb format with hash in front
            {
                $increase_decrease_input_rules .= sprintf('border-color: %1$s !important;', $increaseDecreaseBorderColor);
                $increase_decrease_input_rules .= sprintf('border-style: %1$s !important;', 'solid');
            }

            if ($increaseDecreaseBorderTop != -100) //-100 is the reset value
                $increase_decrease_input_rules .= sprintf('border-top-width: %1$s !important;', $increaseDecreaseBorderTop . 'px');

            if ($increaseDecreaseBorderRight != -100) //-100 is the reset value
                $increase_decrease_input_rules .= sprintf('border-right-width: %1$s !important;', $increaseDecreaseBorderRight . 'px');

            if ($increaseDecreaseBorderBottom != -100) //-100 is the reset value
                $increase_decrease_input_rules .= sprintf('border-bottom-width: %1$s !important;', $increaseDecreaseBorderBottom . 'px');

            if ($increaseDecreaseBorderLeft != -100) //-100 is the reset value
                $increase_decrease_input_rules .= sprintf('border-left-width: %1$s !important;', $increaseDecreaseBorderLeft . 'px');


            if (strlen($qtyBoxBorderColor) == 7) {
                $quantity_input_rules .= sprintf('border-color: %1$s !important;', $qtyBoxBorderColor);
                $quantity_input_rules .= sprintf('border-style: %1$s !important;', 'solid');
            }

            if ($qtyBoxBorderTop != -100)
                $quantity_input_rules .= sprintf('border-top-width: %1$s !important;', $qtyBoxBorderTop . 'px');

            if ($qtyBoxBorderRight != -100)
                $quantity_input_rules .= sprintf('border-right-width: %1$s !important;', $qtyBoxBorderRight . 'px');

            if ($qtyBoxBorderBottom != -100)
                $quantity_input_rules .= sprintf('border-bottom-width: %1$s !important;', $qtyBoxBorderBottom . 'px');

            if ($qtyBoxBorderLeft != -100)
                $quantity_input_rules .= sprintf('border-left-width: %1$s !important;', $qtyBoxBorderLeft . 'px');


            if ($increaseDecreaseHeight > 0) {
                $increase_decrease_input_rules .= sprintf('height: %1$s !important;', $increaseDecreaseHeight . 'px');
                $increase_decrease_input_rules .= sprintf('line-height: %1$s !important;', ($increaseDecreaseHeight - ($increaseDecreaseBorderTop) - $increaseDecreaseBorderBottom) . 'px');
            }
    ...
    public function set_button_text($name)
    {
        ...
        if ($type == 'simple') {
            if (
                ...
                $product->get_stock_quantity() == 0 && !$product->backorders_allowed()
                && trim(Options::bc_atc_get_option($uatcOptions, 'outOfStockText')) !== ''
            ) {
                error_log("product " . $product->get_id() . " is out of stock");
                return Options::bc_atc_get_option($uatcOptions, 'outOfStockText');
            }
